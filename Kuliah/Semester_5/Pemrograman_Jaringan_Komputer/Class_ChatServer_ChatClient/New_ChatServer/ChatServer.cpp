    ...
    while (true)
    {
        ...
        for (int i = 0; i < clientSockets.size(); i++)
        {
            if (FD_ISSET(clientSockets[i], &readSet))
            {
                ...
                MyData receivedData;
                char buffer[sizeof(MyData) + 256]; // Adjust buffer size for message
                ...
                // Print received data
                std::cout << "Received class object data:" << std::endl;
                ...
                // Populate MyData object with the current time and the received message
                MyData newDataToSend;
                newDataToSend.hour = timeInfo->tm_hour;
                newDataToSend.minute = timeInfo->tm_min;
                newDataToSend.second = timeInfo->tm_sec;
                newDataToSend.message = receivedData.message;

                // Serialize the MyData object
                int bufferSize = sizeof(MyData) + receivedData.message.size() + 1;
                char sendBuffer[bufferSize];
                newDataToSend.Serialize(sendBuffer, bufferSize);

                // Echo the MyStruct data back to the client
                send(clientSockets[i], sendBuffer, bufferSize, 0);
            }
        }
    }
