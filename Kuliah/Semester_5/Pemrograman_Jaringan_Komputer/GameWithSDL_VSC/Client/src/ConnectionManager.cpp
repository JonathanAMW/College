...
// bool ConnectionManager::SendData(const GameState& gameState, ClientMode clientMode) {
//     // char buffer[sizeof(GameState)];
//     // gameState.Serialize(buffer, sizeof(buffer);
...
//         return false; // Unknown client mode
//     }
...
// bool ConnectionManager::ReceiveData(GameState& gameState, ClientMode clientMode) {
//     // char buffer[sizeof(GameState)];
...
bool ConnectionManager::SendData(const GameState& gameState, ClientMode clientMode) {
    ...
    if (clientMode == ClientMode::SERVER && serverSocket) {
        int sentBytes = SDLNet_TCP_Send(serverSocket, buffer, sizeof(buffer));
        if (sentBytes <= 0) {
            std::cerr << "Failed to send data: " << SDLNet_GetError() << std::endl;
            return false; // Data not sent
        }
    } else if (clientMode == ClientMode::CLIENT && clientSocket) {
        int sentBytes = SDLNet_TCP_Send(clientSocket, buffer, sizeof(buffer));
        if (sentBytes <= 0) {
            std::cerr << "Failed to send data: " << SDLNet_GetError() << std::endl;
            return false; // Data not sent
        ...
        std::cerr << "Unknown client mode or no socket available" << std::endl;
        return false; // Unknown client mode or no socket available
    }
