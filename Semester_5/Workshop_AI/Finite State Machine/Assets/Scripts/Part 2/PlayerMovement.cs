...
public class PlayerMovement : MonoBehaviour
{
    ...
    public float mJumpHeight = -0.0f;

    [Tooltip(
        "Only useful with Follow and Independent " + "Rotation - third - person camera control"
    )]
        ...
        if (mAnimator != null)
        {
            if (mFollowCameraForward)
            {
                mCharacterController.Move(
                    forward * v * speed * Time.deltaTime + right * h * speed * Time.deltaTime
                );
